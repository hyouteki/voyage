...
Sidebar Sidebar_Init(Vector2 windowSize, Color color) {
	return (Sidebar){.size=Sidebar_Size(windowSize, SidebarDefaultOptions),
					 .color=color, .elements=NULL, .options=SidebarDefaultOptions};
}
...
void Sidebar_AddElement(Sidebar *sidebar, Element *element) {
	int height = ElementList_TotalHeightTill(sidebar->elements, sidebar->options.vPadding, -1);
	Element_ResizeReposition(element, (Vector2){sidebar->options.hPadding, height},
							 (Vector2){sidebar->size.x-2*sidebar->options.hPadding, -1});
...
void Sidebar_Resize(Sidebar *sidebar, Vector2 windowSize) {
	...
	int _counter = 0;
	while (itr) {
		int height = ElementList_TotalHeightTill(sidebar->elements, sidebar->options.vPadding,
												 _counter);
